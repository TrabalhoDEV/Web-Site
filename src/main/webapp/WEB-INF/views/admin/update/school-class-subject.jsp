...
<%
  List<Teacher> availableTeachers  = (List<Teacher>) request.getAttribute("availableTeachers");
  List<Integer> assignedTeacherIds = (List<Integer>) request.getAttribute("assignedTeacherIds");
  ...
  <section>
    <div class="boletim-grid">
      ...
      <div class="header-grid">
        <h1>Editar Professores da Matéria</h1>
      </div>
      ...
      <form method="post" action="${pageContext.request.contextPath}/admin/school-class/subject/update">
        ...
        <div class="form-group">
          ...
          <p class="not-found">Nenhum professor disponível para esta matéria</p>
          <% } else { %>
          <% for (Teacher t : availableTeachers) {
            boolean isAssigned = assignedTeacherIds != null && assignedTeacherIds.contains(t.getId());
          %>
          <label style="display:flex; align-items:center; gap:6px; padding:4px 0; cursor:pointer">
            <input
                    ...
                    <%= isAssigned ? "checked" : "" %>
            />
            <%= OutputFormatService.formatName(t.getName()) %>
          </label>
          <% } %>
          <% } %>
        </div>
        ...
        <div class="modal-actions">
          ...
          <a href="${pageContext.request.contextPath}/admin/school-class/subject/find-many?classId=<%= classId %>"
             class="secondary-button">Cancelar</a>
        </div>
      </form>
    </div>
  </section>
